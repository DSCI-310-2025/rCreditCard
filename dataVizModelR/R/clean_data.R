...
clean_column_names <- function(df) {
  assert_dataframe(df)
  df <- janitor::clean_names(df)  # uses janitor directly
  return(df)
}
...
compute_avg_amounts <- function(df) {
  ...
  # Use dplyr::select without loading the whole package
  df <- dplyr::select(df, -dplyr::all_of(c(bill_amt_cols, pay_amt_cols)))
  return(df)
}
